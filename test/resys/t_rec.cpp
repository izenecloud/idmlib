...
void testKnn(string trainingfile, uint32_t userId, uint32_t itemId)
{
    ...
    KnnItemBasedRecommender knn(trainingModel,NeighborhoodSize);
...
void testKnnNew(char *trainingfile, char* testfile, char* resultfile)
{
    ...
    FileDataModel trainingModel;
    trainingModel.buildDataModel(dataFile);
    KnnItemBasedRecommender test(trainingModel,NeighborhoodSize);
    ...
    for (; iter!=itemPrefs.end(); iter++)
    {
				...
				iterResult++)
		{
			 ...
			 for(uint32_t i=0;i<userRating.size(); i++){
				 ...
				 for (; iter2!=Predictions.end(); iter2++){
...

					{
						 ...
						 MAE+= fabs( diff );
						 if( (userRating[i].getValue() < (iter2->getRating().getAverageValue()+DIFF))

								||(userRating[i].getValue() > (iter2->getRating().getAverageValue()-DIFF)))
								count++;
			 ...
			 cout<<"MAE is  "<<MAE/Predictions.size()<<endl;
		}
