...
template <template <typename VT, typename TI> class MatrixIo, class VT = std::vector<int32_t>, typename IdType = uint32_t, typename ContextIdType = uint32_t, typename FreqType = uint32_t>
...
public:
  ...
  :MatrixIoType(dir+"/matrix"), dir_(dir), rig_(new RIGType(dir_+"/rig"))
  {
  }
  ~RandomIndexing()
  {
