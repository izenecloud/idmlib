...
public:    
        ...
        static double cd(const SCI& item)
        {
            uint32_t s = 0;
            double r = 0;
            double sum = item.f_;
            std::vector<uint32_t> countList;
            KPETermGroup::GetInstance().filter(item.termIdList_, item.contextCountList_, countList);
            if( countList.size() == 0 )
            {
                return 100.0;
            }
        ...
        static double cd2(const SCI& item)
        {
            uint32_t s = 0;
            double r = 0;
            double sum = item.f_;
...
//             std::cout<<"ddd"<<std::endl;
        ...
        {
            double LOGL_MIN = 1.099;
            double d = StatisticalScorer::cd(citem);
            double d2 = StatisticalScorer::cd2(citem);
            std::pair<bool, double> result(true, d2);
            if( d2 < LOGL_MIN )
            {
                result.first = false;
