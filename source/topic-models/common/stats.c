...
double ldf_Bin(int normalize, int x, double pi)
{
    if (x)
    {
        return BOUNDPROB(log(pi));
    }
    else
    {
        return BOUNDPROB(log(1-pi));
    }
}
    ...
    for (i=0;i<=hi1-lo1;i++) for (j=0;j<=hi2-lo2;j++)
        {
            for (k=0;k<=hi3-lo3;k++)
            {
                res[i][j][k] = vec1[i][j][k] * vec2[i][j][k];
                sum += res[i][j][k];
            }
