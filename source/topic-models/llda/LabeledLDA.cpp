...
LabeledLDA::LabeledLDA(const std::string& trainingfile, const std::string& labelfile, int iter)
{
...
void LabeledLDA::load_model()
{
    ...
    w = (int***)malloc(D * sizeof(int**));
    //fprintf(stderr,"D = %d\n", D);
    for (k=0; k<D; k++)
    {
        //fprintf(stderr,"N[%d] = %d\n", k, N[k]);
        w[k] = (int**)malloc(N[k] * sizeof(int*));
        for (i=0; i<N[k]; i++)
        {
            //fprintf(stderr,"M[%d][%d] = %d\n", k, i, M[k][i]);
            w[k][i] = (int*)malloc(M[k][i] * sizeof(int));
        }
    }
...
void LabeledLDA::resample_post_beta()
{
    int k_8;
    double* tmpSP4;
    int d_2;
...
void LabeledLDA::resample_post_theta()
{
    int d_9;
    double* tmpSP7;
    int n_3;
...
void LabeledLDA::initialize_alpha()
{
    alpha = sample_Gam(1.0, 1.0);
}

void LabeledLDA::initialize_eta()
{
    eta = sample_Gam(1.0, 1.0);
}

void LabeledLDA::initialize_z()
{
    int d_10;
    int n_16;
    int m_22;
...
void LabeledLDA::initialize_post_beta()
{
    int dvv_loop_var_1;
    int dvv_loop_var_2;
    for (dvv_loop_var_1=1; dvv_loop_var_1<=K; dvv_loop_var_1++)
...
void LabeledLDA::initialize_post_theta()
{
    int dvv_loop_var_1;
    int dvv_loop_var_2;
    for (dvv_loop_var_1=1; dvv_loop_var_1<=D; dvv_loop_var_1++)
...
double LabeledLDA::compute_log_posterior()
{
    double ldfP4_0;
    double ldfP4_1;
    double ldfP4_2;
    ...
    for (d_10=1; d_10<=D; d_10++)
    {
        ...
        for (n_16=1; n_16<=N[d_10-1]; n_16++)
        {
            ...
            for (m_22=1; m_22<=M[d_10-1][n_16-1]; m_22++)
            {
                ldfP4_2 += ldf_Mult(1, z[d_10-1][n_16-1][m_22-1], post_theta[d_10-1], 1, K);
            }
            ldfP4_1 += ldfP4_2;
        }
        ldfP4_0 += ldfP4_1;
    ...
    for (d_11=1; d_11<=D; d_11++)
    {
        ...
        for (n_17=1; n_17<=N[d_11-1]; n_17++)
        {
            ...
            {
                ldfP5_2 += ldf_Mult(1, w[d_11-1][n_17-1][m_23-1], post_beta[z[d_11-1][n_17-1][m_23-1]-1], 1, V);
            }
            ldfP5_1 += ldfP5_2;
        }
        ldfP5_0 += ldfP5_1;
...
void LabeledLDA::estimate()
{
    initialize_model();

    initialize_z();
    initialize_post_beta();
    initialize_post_theta();

    double loglik,bestloglik=0;
...
void LabeledLDA::initialize_model()
{
    ...
    for (int malloc_dim_1=1; malloc_dim_1<=D; malloc_dim_1++)
    {
        ...
        for (int malloc_dim_2=1; malloc_dim_2<=N[malloc_dim_1-1]; malloc_dim_2++)
        {
