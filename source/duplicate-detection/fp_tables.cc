#include <idmlib/duplicate-detection/fp_tables.h>

using namespace idmlib::dd;

void FpTables::GenTables(uint32_t f, uint8_t k, uint8_t partition_num, std::vector<FpTable>& table_list) const
{
    ...
    do
    {
        ...
        FpType& bit_mask = table_list.back().bit_mask_;
        for (uint8_t i = 0; i < len; i++)
        {
...
bool FpTables::permutate_(std::vector<uint8_t>& vec, uint32_t index, uint32_t size) const
{
