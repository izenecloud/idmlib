/**
 * Name        : UtilProperty.cpp
 * Description : Used to read the parameters from property configuring file.
*/
...
void UtilProperty::read(const char *filename, PropertyMapT &map)
{
    std::ifstream file(filename);
    if (!file)
          {
        printf("unable to open properties file");
            return;
           }
    read(file, map);
    file.close();
...
void UtilProperty::read(std::istream &is, PropertyMapT &map)
{
    if (!is)
          {
        printf("unable to read from stream");
          return;
          }
...
void UtilProperty::write(const char *filename, PropertyMapT &map, const char *header)
{
...
void UtilProperty::write(std::ostream &os, PropertyMapT &map, const char *header)
{
    if (header != NULL)
        os << '#' << header << std::endl;
...
void UtilProperty::print(std::ostream &os, PropertyMapT &map)
{
